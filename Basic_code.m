...
plotCohensD_bivariate_byGroup(cohensD,cohensD_rawpain, Group_labels, ...
    'GroupRegions', true, 'GroupRegionStyle', 'ellipse','GroupRegionAlpha', 0.08,'XLim', [-1.5 1], 'YLim', [-2.75 0.5],'XLabel','NPS','YLabel','Pain');
...
[figOut, ~] = plotCohensD_byGroup(s_val_mat(ord,:), Group_labels(ord), ...
    ...         % tells function to plot into this subplot
    'Colors', cl_mat, ...        % supply full color matrix matching canonical labels
    'Sorting', false, ...
    'ShowLegend', true,...
...
[tbl, X, groupVec, varNames] = createMixedDesignEffectTable(Group_labels, cohensD.n,[],[],'referenceLevel','Cognitive');
...
% Fixed effects
tbl.jkc  = sim_vect;
lme2 = fitlme(tbl, 'Y ~ 1+ Label_Conditioning + Label_Mindfulness + Label_Placebo + Label_Placebo_ + Label_Remifentanil + Label_Social + jkc+ (1|StudyID)');
disp(lme2);
[~,~,stats] = fixedEffects(lme2,'dfmethod','satterthwaite')

% Weighed regression
lme2 = fitlme(tbl, 'Y ~ 1+ Label_Conditioning + Label_Mindfulness + Label_Placebo + Label_Placebo_ + Label_Remifentanil + Label_Social + (1|StudyID)','Weights',(sim_vect+1)/2);
disp(lme2);
[~,~,stats] = fixedEffects(lme2,'dfmethod','satterthwaite')

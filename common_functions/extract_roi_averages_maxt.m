function cl = extract_roi_averages_maxt(obj, mask, varargin)
...
for i = 1:nregions
    ...
    if ~isempty(regiondat)
        if size(regiondat, 2) == 1              % only one image, mean is image val
            ...
            regionmean = max_tscore_roi(regiondat, 1.98, 5);
            % regionmean = double(nanmean(regiondat, 1)); % mean over images
            % regionmean = double(median(regiondat, 2,'omitmissing')); % mean over images
        end
...
function roi_summary = max_tscore_roi(regiondat, t_thresh, k_min)
% MAX_TSCORE_ROI   for each image (row) of regiondat, 
%                  return max(t) if ≥k_min voxels exceed t_thresh,
%                  else return 0.
%
% regiondat : nImages×nVoxels_inROI matrix of voxel-t values
% t_thresh  : scalar voxel-wise threshold
% k_min     : minimum number of voxels required
%
    nImgs       = size(regiondat, 1);
    roi_summary = zeros(1, nImgs);
    for img = 1:nImgs
        ts   = regiondat(img, :);
        sup  = abs(ts) > t_thresh;
        if nnz(sup) >= k_min
            roi_summary(img) = max(abs(ts(sup))).*sign(median(ts(sup)));
        else
            roi_summary(img) = 0;
        end
    end
